...
function actuTabla(){
    var htmlTabla="";
    $.ajax({
        url:'php/publicacionesListado.php',
        type: 'POST',
        async: false,
        success: function(data){
            var linea;
            ...
            $.each(JSON.parse(data), (key,  imagen)=>{
                ...
                linea ='<tr>\
                    <td class="primary">\
                    <a href="https://smg.gt/publicaciones/files/'+imagen.fecha+'/ee/'+imagen.nombre+'" target="_blank">\
...
function nombre(ruta, nombre){
    ...
    function(){
        ...
        $.ajax({
            ...
            success: function(data){
                if(data==0){
                    alertify.success('El nombre del archivo ha sido cambiado')
                    //location.reload();
                    actuTabla();
                    $("#tabla").trigger("destroy");
                    tableSort();
